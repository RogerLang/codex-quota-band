...
internal data class Stage03CAutoReturnResult(
  val sequence: Int,
  ...
  val overall: String
    get() = when {
      ...
      messageResult == "ACK_PERSISTED" -> "AUTO_RETURN_PERSIST_REQUESTED"
...
internal class Stage03CRunner(context: Context) {
  ...
  suspend fun wakeSendAndReturn(sequence: Int = 46): Stage03CAutoReturnResult {
    ...
    if (query.error != null) {
      return Stage03CAutoReturnResult(sequence, serviceResult(query.error), "NOT_SENT", nodeAttempt = query.attempt)
    }
    val node = query.value.orEmpty().firstOrNull { isBand9ProNodeName(it.name) }
      ?: return Stage03CAutoReturnResult(sequence, "NODE_NOT_FOUND", "NOT_SENT", nodeAttempt = query.attempt)

    if (!ensurePermission(node.id, Permission.DEVICE_MANAGER)) {
      return Stage03CAutoReturnResult(sequence, "DEVICE_MANAGER_DENIED", "NOT_SENT", nodeAttempt = query.attempt)
    }
    ...
    try {
      try {
        ...
        return Stage03CAutoReturnResult(sequence, "LISTENER_FAILED", "NOT_SENT", nodeAttempt = query.attempt)
      }
      ...
      for (attempt in 1..MAX_SEND_ATTEMPTS) {
        try {
          ...
          if (attempt == MAX_SEND_ATTEMPTS) {
            return Stage03CAutoReturnResult(sequence, launch, "SEND_FAILED", attempt, query.attempt)
          }
        ...
        if (ack != null) {
          return Stage03CAutoReturnResult(
            sequence = sequence,
            launchResult = launch,
      ...
      return Stage03CAutoReturnResult(sequence, launch, "ACK_TIMEOUT", MAX_SEND_ATTEMPTS, query.attempt)
    } finally {
      if (listening) {
